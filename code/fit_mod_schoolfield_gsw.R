...
fit_mod_schoolfield_gsw <- function(Data, curveID = "curveID", x = "Tleaf", y = "gsw", T_ref = 25, fitted_models = TRUE) {
  models <- c("schoolfield")
  aic_values <- list()
  fits <- list()
  Data$T_ref = T_ref
  pdf("schoolfield_gsw_fits.pdf")
  ...
  for (curve_id in unique(Data[[curveID]])) {
    ...
    for (model in models) {
      fit <- tryCatch({
        nls.multstart::nls_multstart(gsw ~ schoolfield(temp = Tleaf, J_ref, E, E_D, T_opt, T_ref),
                                     data = curve_data,
                                     iter = c(2,2,2,2),
      ...
      if (!is.null(fit)) {
        ...
        r_sq <- 1 - sum(resid(fit)^2) / sum((curve_data$gsw - mean(curve_data$gsw))^2)
        ...
        # --- New breadth calc: 95% of gswmax ---
        gswmax <- max(df$gsw, na.rm = TRUE)
        gsw95 <- 0.95 * gswmax
        ...
        # Find T at which gsw crosses 95% of gswmax on each side
        Tlower <- left_df$Tleaf[which.min(abs(left_df$gsw - gsw95))]
        Tupper <- right_df$Tleaf[which.min(abs(right_df$gsw - gsw95))]
...
#schoolfield.results.data=fit_mod_schoolfield(at.subset3%>% select(Tleaf, gsw, curveID), x = "Tleaf", y = "gsw", T_ref = 25)
#sum(schoolfield.results.data$gswIC) # = -37008.26
