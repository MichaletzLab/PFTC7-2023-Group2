...
dat <- dat %>% mutate(fspecies = factor(species), fsite = factor(site))

vec_tleaf <- range(dat$tleaf, na.rm = TRUE)
vec_cond <- range(dat$cond, na.rm = TRUE)
...
ref_dat <- dat[, .(photo_u = mean(photo, na.rm = TRUE), photo_sd = sd(photo)), by = species]
dat <- merge(dat, ref_dat, by = 'species', all.x = TRUE)

dat[, `:=`(photo_rel = (photo - photo_u) / photo_u, photo_z = (photo - photo_u) / photo_sd)]
...
# Fit GAM models for Conductance (cond) with smoothing terms for fspecies, cond, and tleaf
vv <- vec_mod_group %>% 
...
#combined_df <- rbind(vv2, ww2, fill=TRUE)
...
ggsave(p_out,
       filename=
         paste0("figures/plot_photo_gs_site_tv_with-re_cond-effect","_",
                Sys.Date(),
                ".png"),
       width=22,
       height=12,
...
#Need to make it so that just vv or ww runs and only make one fig. right now they are the same thing since
##we can't do effect of cond on cond obv.
