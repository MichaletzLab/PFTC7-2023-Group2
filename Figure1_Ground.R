cat_colors <- c("Low" = "grey40", "Medium" = "lightblue3", "High" = "#3333CC")

gam_A_T2   <- gam(A   ~ T2_cat + s(Tleaf, by = T2_cat, k = 10), data = dat.T2Temp)
gam_E_T2   <- gam(E   ~ T2_cat + s(Tleaf, by = T2_cat, k = 10), data = dat.T2Temp)
gam_gsw_T2 <- gam(gsw ~ T2_cat + s(Tleaf, by = T2_cat, k = 10), data = dat.T2Temp)

pred_grid_T2 <- dat.T2Temp %>%
  group_by(T2_cat) %>%
  summarise(Tleaf = seq(min(Tleaf, na.rm=TRUE), max(Tleaf, na.rm=TRUE), length.out = 200),
            .groups = "drop")

add_preds <- function(pred_df, gam_obj, resp_name){
  pr <- predict(gam_obj, newdata = pred_df, se.fit = TRUE, type = "response")
  pred_df %>%
    mutate(!!paste0(resp_name,"_fit")   := pr$fit,
           !!paste0(resp_name,"_lower") := pr$fit - pr$se.fit,
           !!paste0(resp_name,"_upper") := pr$fit + pr$se.fit)
}

pred_full_T2 <- pred_grid_T2 %>%
  add_preds(gam_A_T2,   "A") %>%
  add_preds(gam_E_T2,   "E") %>%
  add_preds(gam_gsw_T2, "gsw")

plot_gam3 <- function(df_points, df_pred, cat_var, yvar, fit, lo, hi, ylab, color_name){
  ggplot() +
    geom_point(data = df_points, aes(x = Tleaf, y = .data[[yvar]], color = .data[[cat_var]]), alpha = 0.01) +
    geom_ribbon(data = df_pred, aes(x = Tleaf, ymin = .data[[lo]], ymax = .data[[hi]], fill = .data[[cat_var]]),
                alpha = 0.2) +
    geom_line(data = df_pred, aes(x = Tleaf, y = .data[[fit]], color = .data[[cat_var]]), size = 1.2) +
    scale_color_manual(values = cat_colors, name = color_name) +
    scale_fill_manual(values = cat_colors,  name = color_name) +
    labs(x = "Leaf temperature (°C)", y = ylab) +
    theme_classic()
}

pA_full_T2 <- plot_gam3(dat.T2Temp, pred_full_T2, "T2_cat", "A",   "A_fit",   "A_lower",   "A_upper",
                        expression(A~(mu*mol~CO[2]~m^-2~s^-1)), "Ground temperature (T2)")
pE_full_T2 <- plot_gam3(dat.T2Temp, pred_full_T2, "T2_cat", "E",   "E_fit",   "E_lower",   "E_upper",
                        expression(E~(mmol~m^-2~s^-1)),          "Ground temperature (T2)")
pg_full_T2 <- plot_gam3(dat.T2Temp, pred_full_T2, "T2_cat", "gsw", "gsw_fit", "gsw_lower", "gsw_upper",
                        expression(g[s][w]~(mol~m^-2~s^-1)),     "Ground temperature (T2)")

fig1_T2 <- ggarrange(pA_full_T2, pE_full_T2, pg_full_T2,
                     nrow = 1, ncol = 3, common.legend = TRUE, legend = "top",
                     labels = c("A","B","C"))

# HOT subset
hot_T2 <- dat.T2Temp %>% filter(Tleaf > TOPT)

gam_A_hot_T2   <- gam(A   ~ T2_cat + s(Tleaf, by = T2_cat, k = 10), data = hot_T2)
gam_E_hot_T2   <- gam(E   ~ T2_cat + s(Tleaf, by = T2_cat, k = 10), data = hot_T2)
gam_gsw_hot_T2 <- gam(gsw ~ T2_cat + s(Tleaf, by = T2_cat, k = 10), data = hot_T2)

pred_hot_grid_T2 <- hot_T2 %>%
  group_by(T2_cat) %>%
  summarise(Tleaf = seq(min(Tleaf, na.rm=TRUE), max(Tleaf, na.rm=TRUE), length.out = 200),
            .groups = "drop")

pred_hot_T2 <- pred_hot_grid_T2 %>%
  add_preds(gam_A_hot_T2,   "A") %>%
  add_preds(gam_E_hot_T2,   "E") %>%
  add_preds(gam_gsw_hot_T2, "gsw")

deriv_by_cat <- function(gam_obj, data_ref, cat_var){
  levs <- levels(data_ref[[cat_var]])
  out <- lapply(levs, function(z){
    nd <- data_ref %>% filter(.data[[cat_var]] == z) %>% select(Tleaf, all_of(cat_var)) %>% distinct() %>% arrange(Tleaf)
    if(nrow(nd) == 0) return(NULL)
    dd <- derivatives(gam_obj, term = paste0("s(Tleaf):", cat_var, z), newdata = nd, partial_match = TRUE)
    dd[[cat_var]] <- z
    dd
  })
  bind_rows(out)
}

derA_T2   <- deriv_by_cat(gam_A_hot_T2,   pred_hot_grid_T2, "T2_cat")
derE_T2   <- deriv_by_cat(gam_E_hot_T2,   pred_hot_grid_T2, "T2_cat")
dergsw_T2 <- deriv_by_cat(gam_gsw_hot_T2, pred_hot_grid_T2, "T2_cat")

plot_hot <- function(df_points, df_pred, cat_var, yvar, fit, lo, hi, ylab, color_name){
  ggplot() +
    geom_point(data = df_points, aes(x = Tleaf, y = .data[[yvar]], color = .data[[cat_var]]), alpha = 0.02) +
    geom_ribbon(data = df_pred, aes(x = Tleaf, ymin = .data[[lo]], ymax = .data[[hi]], fill = .data[[cat_var]]),
                alpha = 0.2) +
    geom_line(data = df_pred, aes(x = Tleaf, y = .data[[fit]], color = .data[[cat_var]]), size = 1.2) +
    scale_color_manual(values = cat_colors, name = color_name) +
    scale_fill_manual(values = cat_colors,  name = color_name) +
    labs(x = "Leaf temperature (°C)", y = ylab) +
    theme_classic()
}

plot_deriv <- function(deriv_df, cat_var, ylab, color_name){
  ggplot(deriv_df, aes(x = Tleaf, y = .derivative, color = .data[[cat_var]])) +
    geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci, fill = .data[[cat_var]]), alpha = 0.15, color = NA) +
    geom_line(size = 1.1) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    scale_color_manual(values = cat_colors, name = color_name) +
    scale_fill_manual(values = cat_colors,  name = color_name) +
    labs(x = "Leaf temperature (°C)", y = ylab) +
    theme_classic()
}

pA_hot_T2  <- plot_hot(hot_T2, pred_hot_T2, "T2_cat", "A",   "A_fit",   "A_lower",   "A_upper",
                       expression(A~(mu*mol~CO[2]~m^-2~s^-1)), "Ground temperature (T2)")
pE_hot_T2  <- plot_hot(hot_T2, pred_hot_T2, "T2_cat", "E",   "E_fit",   "E_lower",   "E_upper",
                       expression(E~(mmol~m^-2~s^-1)),          "Ground temperature (T2)")
pg_hot_T2  <- plot_hot(hot_T2, pred_hot_T2, "T2_cat", "gsw", "gsw_fit", "gsw_lower", "gsw_upper",
                       expression(g[s][w]~(mol~m^-2~s^-1)),     "Ground temperature (T2)")

pA_d1_T2   <- plot_deriv(derA_T2,   "T2_cat",
                         expression(frac(dA,dT[leaf])~~"(" * mu*mol~CO[2]~m^-2~s^-1~degree*C^-1 * ")"),
                         "Ground temperature (T2)")
pE_d1_T2   <- plot_deriv(derE_T2,   "T2_cat",
                         expression(frac(dE,dT[leaf])~~"(" * mmol~m^-2~s^-1~degree*C^-1 * ")"),
                         "Ground temperature (T2)")
pgsw_d1_T2 <- plot_deriv(dergsw_T2, "T2_cat",
                         expression(frac(dg[s][w],dT[leaf])~~"(" * mol~m^-2~s^-1~degree*C^-1 * ")"),
                         "Ground temperature (T2)")

fig1_T2
fig2_T2 <- ggarrange(pA_hot_T2, pE_hot_T2, pg_hot_T2, pA_d1_T2, pE_d1_T2, pgsw_d1_T2,
                     nrow = 2, ncol = 3, common.legend = TRUE, legend = "top",
                     labels = c("A","B","C","D","E","F"))
fig2_T2
