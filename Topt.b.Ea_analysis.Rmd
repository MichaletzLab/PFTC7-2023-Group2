...
discard.weibull = read_csv("outputs/weibull.discard.hooks.csv")

discard.schoolfield = read_csv("outputs/schoolfield.discard.hooks.csv")
```
...
discard.weibull <- read_csv("discard.weibull.SANW.csv")
discard.schoolfield <- read.csv("discard.schoolfield.SANW.csv")
```
...
```{r}
elev.asp.plot = ggplot(discard.weibull, aes(x = Elevation, y = T_opt, shape = Aspect)) +
  ...
  labs(shape = "Aspect") +
  scale_shape_manual(values = c("E" = 1, "W" = 2), labels = c("East", "West")) +
  theme_classic() +
  xlab("Elevation (m)") + 
  ylab("Topt (˚C)") +
  theme(
...
model.toptelev <- lm(T_opt ~ Elevation+Aspect, data = discard.weibull)
