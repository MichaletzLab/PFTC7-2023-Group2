...
# Load fit functions:
source("FigureS1and2_VPDanalysis/ModelCode/GAM_schoolfield_tleaf.R")
source("FigureS1and2_VPDanalysis/ModelCode/GAM_schoolfield.R")
source("FigureS1and2_VPDanalysis/ModelCode/GAM_schoolfield_tleaf_cond.R")
########################################################
...
#####################################################
# Fit Schoolfield GAM --> run models in the ModelCode folder first.
ssmod <- lapply(vec_mod_group, function(sel_group) {
  tmp_dat <- dat[dat$mod_group == sel_group, ]
  fit_results <- fit_schoolfieldONLY_gam("tleaf", "photo", T_ref = 25, start_params = start_params, data = tmp_dat)
...
# Calculate Average Schoolfield Topt
avg_school_topt <- dat %>%
