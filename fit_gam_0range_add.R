...
fit_gam <- function(data, response, predictors) {
  gam_formula <- as.formula(paste(response, "~", paste(sapply(predictors, function(x) paste("s(", x, ", bs = 'ts', k = 5, m = c(2, 0), xt = c(0, 40))")))))
  fit <- gam(gam_formula, data = data, select = TRUE, method = "REML")
  return(fit)
}

#gam(photo ~ s(tleaf, bs='ts', k=5, sp = list(bs = 'ts', k = 5, m = c(2, 0), xt = c(0, 40))), data = curve_data, method = "REML")
